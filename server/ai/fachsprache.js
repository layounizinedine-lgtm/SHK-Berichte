...
/** Heizkörpertyp aus dem gesprochenen Typwort ableiten (für Baulänge-Angaben). */
function heizkoerperTyp(typWort) {
  const w = String(typWort || '').toLowerCase();
  if (/^(guss|guß)/.test(w)) return { fach: 'Gussheizkörper', werkstoff: 'heizkoerper_guss' };
  if (/^(röhren|roehren)/.test(w)) return { fach: 'Röhrenheizkörper (Röhrenradiator)', werkstoff: 'heizkoerper_roehren' };
  if (/^platten/.test(w)) return { fach: 'Plattenheizkörper', werkstoff: 'heizkoerper_platten' };
  if (/^kompakt/.test(w)) return { fach: 'Kompaktheizkörper', werkstoff: 'heizkoerper_kompakt' };
  if (/^(design|handtuch|bad)/.test(w)) return { fach: 'Design-/Badheizkörper (Handtuchheizkörper)', werkstoff: 'heizkoerper_design' };
  return { fach: 'Heizkörper', werkstoff: 'heizkoerper_unbekannt' };
}
function heizkoerperFach(typWort, laengeCm) {
  const { fach } = heizkoerperTyp(typWort);
  return laengeCm ? `${fach} (Baulänge ${laengeCm} cm)` : fach;
}
const HEIZKOERPER_TYPWORT = '(guss|guß|gusseisern\\w*|gußeisern\\w*|röhren\\w*|roehren\\w*|platten\\w*|kompakt\\w*|design\\w*|handtuch\\w*|bad)';
...
const DIMENSIONS_REGELN = [
  // "200cm Gussheizkörper" / "200 cm Heizkörper"
  {
    re: new RegExp(`\\b(\\d{2,3})\\s*(?:cm|zentimeter)\\s*${HEIZKOERPER_TYPWORT}?\\s*heizk[oö]rper\\w*\\b`, 'gi'),
    fach: (m) => heizkoerperFach(m[2], m[1]),
    kategorie: 'bauteil',
    werkstoff: (m) => heizkoerperTyp(m[2]).werkstoff,
  },
  // "Gussheizkörper 200cm" / "Heizkörper mit 200 cm"
  {
    re: new RegExp(`\\b${HEIZKOERPER_TYPWORT}?\\s*heizk[oö]rper\\w*\\s*(?:mit\\s*)?(\\d{2,3})\\s*(?:cm|zentimeter)\\b`, 'gi'),
    fach: (m) => heizkoerperFach(m[1], m[2]),
    kategorie: 'bauteil',
    werkstoff: (m) => heizkoerperTyp(m[1]).werkstoff,
  },
...
export function normalisiere(eingabe) {
  ...
  for (const regel of DIMENSIONS_REGELN) {
    text = text.replace(regel.re, (...args) => {
      ...
      const werkstoff = typeof regel.werkstoff === 'function' ? regel.werkstoff(m) : regel.werkstoff;
      treffer.push({ roh: m[0].trim(), fach, kategorie: regel.kategorie, werkstoff });
      return merke(fach);
    });
  }
  ...
  // Heizkörper: Werkzeug/Material je Typ, Rückfrage bei unbekanntem Typ
  for (const t of treffer) {
    if (t.kategorie !== 'bauteil' || !String(t.werkstoff || '').startsWith('heizkoerper')) continue;
    if (t.werkstoff === 'heizkoerper_guss') {
      werkzeug.add('Rohrzange'); werkzeug.add('Wasserpumpenzange'); werkzeug.add('Auffangwanne bzw. Restwasserbehälter');
      material.add('Dichtungssatz für Heizkörperanschluss'); material.add('Entlüftungsventil');
      normen.add('DIN EN 12828');
      hinweise.add('Gussheizkörper sind schwer und spröde – beim Aus- und Einbau Tragehilfe bzw. zweite Person einsetzen und Anschlussgewinde nicht überdrehen; fachgerechte Entsorgung als Alt-/Schrottmetall.');
    } else if (t.werkstoff === 'heizkoerper_roehren' || t.werkstoff === 'heizkoerper_platten' || t.werkstoff === 'heizkoerper_kompakt' || t.werkstoff === 'heizkoerper_design') {
      werkzeug.add('Rohrzange'); werkzeug.add('Wasserpumpenzange');
      material.add('Dichtungssatz für Heizkörperanschluss');
      normen.add('DIN EN 442'); normen.add('DIN EN 12828');
    } else if (t.werkstoff === 'heizkoerper_unbekannt') {
      hinweise.add('Heizkörpertyp nicht genannt – bitte ergänzen (Guss-, Röhren-, Platten-, Kompakt- oder Designheizkörper) für einen präziseren Eintrag.');
    }
  }
